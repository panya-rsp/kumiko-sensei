# Kumiko Sensei (Kumiko Knowledge Desk) — build and run
...
```sh
open KumikoKnowledgeDesk.xcodeproj
```

Select the `KumikoKnowledgeDesk` scheme and press Run. On first launch choose your Cheatbook folder or click **Try the sample library** (installs `PreviewLibrary/` into the app's container).

## Command line

```sh
# Build the sandboxed app
xcodebuild -project KumikoKnowledgeDesk.xcodeproj -scheme KumikoKnowledgeDesk \
...
xcodebuild -project KumikoKnowledgeDesk.xcodeproj -scheme KumikoKnowledgeDesk \
  ...
  -only-testing:KumikoKnowledgeDeskTests test
...
.build-shots/Build/Products/Debug/Kumiko\ Sensei.app/Contents/MacOS/KumikoKnowledgeDesk \
...
`KumikoKnowledgeDeskUITests` drives search, detail, image inspector, inbox, composer, and Ask Kumiko, and saves screenshots to `$KUMIKO_SHOTS_DIR`. It needs the macOS Automation permission prompt to be accepted once:
...
xcodebuild -project KumikoKnowledgeDesk.xcodeproj -scheme KumikoKnowledgeDesk \
  ...
  -only-testing:KumikoKnowledgeDeskUITests test
```
...
KumikoKnowledgeDeskTests/   Swift Testing unit tests (fixture: PreviewLibrary/)
KumikoKnowledgeDeskUITests/ XCUITest walkthrough
